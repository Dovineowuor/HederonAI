...
Keep it practical and measurable (150-200 words). Use bullet points.`,
...
export async function POST(req: NextRequest) {
  ...
  if (mode === "challenge") {
    const result = await handleChallengeWorkflow(challenge, kiloAvailable, logs);
    return NextResponse.json(result);
  } else {
    const result = await handleGoalExecution(goal, kiloAvailable, logs);
    return NextResponse.json(result);
  }
}
